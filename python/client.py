...
def click(selector):
    px.Action(clickAction=px.ClickAction(selector=selector))

def select(selector, values=[]):
    px.Action(selectAction=px.SelectAction(selector=selector, values=values))

def type(selector, text):
    px.Action(typeAction=px.TypeAction(selector=selector, text=text)),

def get_text(selector):
    px.Action(getInnerTextAction=px.GetInnerTextAction(selector=selector)),
...
def do(stub):
    actions = [
        px.Action(launchAction=px.LaunchAction(headless=False)),
        px.Action(gotoAction=px.GotoAction(
